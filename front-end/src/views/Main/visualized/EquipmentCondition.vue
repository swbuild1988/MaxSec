...
export default class EquipmentCondition extends Vue {
        ...
        getEquipmentTypeStateStatistice()
            ...
            .finally(() => {
                ...
                for (let index = 0; index < this.charts.length; index++) {
                    this.initChart(
                        this.charts[index].id,
                        this.charts[index].typeId
                    );
                }
            });
    }
    ...
    initChart(id: string, typeId: number) {
        // 先找出这个图应该有的类型
        let typeIndex: number = this.equipmentTypes.findIndex(
            (type: EnumType) => type.val == typeId
        );
        ...
        // 找出这个图的数据
        let dataIndex: number = this.statisticeData.findIndex(
            (d: any) => d.key == type.key
        );
    ...
    clickEvent(params: any) {
        let typeIndex: number = this.equipmentTypes.findIndex(
            (type: EnumType) => type.key == params.seriesName
        );
        if (typeIndex < 0) return;
