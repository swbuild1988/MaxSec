...
import { Card, Department } from "@/types/intercom.interface";
import { getVideoDtosByCondition, getDefaultVideo } from "@/api/video";
import { getCards, getDepartments } from "@/api/intercom";
...
export default class VideoIntercom extends Vue {
    ...
    refreshData() {
        ...
        let vo: AlarmVo = {
            ...
            pageSize: 20,
            objId: this.video.id,
            startTime: DateUtil.TodayStartTime(),
            endTime: DateUtil.TodayEndTime(),
        };
    ...
    /** 刷新成功认证图像 */
    refreshSuccessImage() {
        let successAlarm: Alarm[] = this.alarms.filter(
            (alarm: Alarm) => alarm.alarmTypeId == 3001
        );

        this.alarm1.length = 0;
        for (let i = 0; i < Math.min(successAlarm.length, this.imgNum); i++) {
            this.alarm1.push(successAlarm[i]);
        }
    }

    /** 刷新失败认证图像 */
    refreshFailImage() {
        let failAlarm: Alarm[] = this.alarms.filter(
            (alarm: Alarm) => alarm.alarmTypeId == 3002
        );

        this.alarm2.length = 0;
        for (let i = 0; i < Math.min(failAlarm.length, this.imgNum); i++) {
            this.alarm2.push(failAlarm[i]);
        }
    }
