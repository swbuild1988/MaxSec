<template>
    <div class="production_area_west">
        <h4>{{ videoName }}</h4>
        <div id="OCXOverviewCenterBody">
...
@Component({})
export default class ProductionAreaWest extends Vue {
    videoName: String = "";
    m_bDVRControl: any = "";
    ...
    mounted() {
        this.getVideosInfo();
        this.m_bDVRControl = document.getElementById("HIKOBJECT2");
        this.ChangeStatus();
    }
    ...
    ChangeStatus() {
        document.getElementById("NetPlayOCX2").style.border = "1px solid #00F";
    }
    ...
    getVideosInfo() {
        getVideoDtos().then((res: any) => {
            ...
            if (code != 200) {
                ...
                this.videoName = this.videoListArr[1].name;
                if (!this.videoListArr) {
                    return;
                }
                // 开始预览
                this.PreviewStart(this.videoListArr[1]);
            }
        });
    }
